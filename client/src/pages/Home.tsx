...
type SelectedScenario = ScenarioKey | "custom";
...
export default function Home() {
  ...
  const [selectedScenario, setSelectedScenario] = useState<SelectedScenario>("malicious");
  ...
  const runFirewall = () => {
    if (isRunning) return;
    const request = { prompt, tool, result };
    setIsRunning(true);
    setWhyBlocked(false);
    setPipelineStage(0);
    let stage = 0;
    timerRef.current = window.setInterval(() => {
      ...
      if (stage >= stages.length - 1) {
        ...
        setAudit((current) => [{ time, request: request.prompt, tool: request.tool, risk: request.result.risk, threats: request.result.threats.map((threat) => threat.label).join(", ") || "None", decision: request.result.decision }, ...current].slice(0, 8));
        setIsRunning(false);
      }
    }, 430);
  };
  ...
  return (
    <main className="console-shell">
      ...
      <section className="section-pad simulator-section" id="simulator">
        ...
        <div className="scenario-picker"><div className="scenario-picker-label"><Zap size={14} /> DEMO SCENARIOS</div>{(Object.keys(scenarios) as ScenarioKey[]).map((key) => <button type="button" key={key} className={`scenario-chip ${selectedScenario === key ? "selected" : ""}`} onClick={() => loadScenario(key)} disabled={isRunning}><span className={`scenario-dot dot-${key}`} /><span><strong>{scenarios[key].label}</strong><small>{scenarios[key].description}</small></span><ChevronRight size={14} /></button>)}</div>
        <div className="simulator-grid">
          <div className="simulator-card input-card"><div className="card-header"><span><Terminal size={15} /> REQUEST ENVELOPE</span><Badge>UNTRUSTED INPUT</Badge></div><div className="form-field"><label htmlFor="prompt">User prompt / task</label><textarea id="prompt" value={prompt} disabled={isRunning} onChange={(event) => { setPrompt(event.target.value); setSelectedScenario("custom"); setPipelineStage(0); }} placeholder="Describe the task the agent should perform..." /></div><div className="field-row"><div className="form-field"><label htmlFor="intent">Declared intent</label><select id="intent" value={intent} disabled={isRunning} onChange={(event) => { setIntent(event.target.value); setSelectedScenario("custom"); setPipelineStage(0); }}><option>Read public report</option><option>Write internal record</option><option>Send external message</option></select></div><div className="form-field"><label htmlFor="tool">Requested tool</label><select id="tool" value={tool} disabled={isRunning} onChange={(event) => { setTool(event.target.value); setSelectedScenario("custom"); setPipelineStage(0); }}><option value="send_email">send_email</option><option value="read_document">read_document</option><option value="execute_code">execute_code</option><option value="export_data">export_data</option></select></div></div><button type="button" className="simulate-button" onClick={runFirewall} disabled={isRunning}><Play size={15} fill="currentColor" /> {isRunning ? "Running pipeline..." : "Run through firewall"}</button></div>
